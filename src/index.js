    ...
    execute: function (message, args) {
        ...
        axios.get(`https://en.wikipedia.org/api/rest_v1/page/${args[0] == undefined ? "random/summary" : "summary/" + args.join("_").replace("/", "%2F")}`)
			.then(json => {
				...
				if(isDisambiguation) return message.reply({ content: `**${json.data.title}** refers to multiple things. Try something more specific listed here: <${json.data.content_urls.desktop.page}>`, allowedMentions: { parse: [] }});
				if(isSection) {
					const sectionName = new URL(json.request.res.responseUrl).hash;
					return message.reply({ content: `It looks like you have been redirected to a section of an article, or tried to access one. Sections are not currently supported.\nPlease view the page at <${json.data.content_urls.desktop.page + sectionName}>.`, allowedMentions: { parse: [] }});
				}
